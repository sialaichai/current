...
const CONFIG = {
  ...
  moveSpeed: 5,
  climbSpeed: 4,
  energyDrainRate: 0.02, // per frame (~1/sec at 60fps)
  initialRobots: 4,
  initialEnergy: 100,
  levels: 22,
  levelNames: [
...
class Player {
  ...
  update(input, platforms, ladders, canvasHeight) {
    ...
    // If not climbing, apply vertical movement and check platforms
    if (!this.climbing) {
      this.y = nextY;
    ...
    // Prevent falling through bottom (emergency floor)
    const floorY = canvasHeight - 20; // ground platform height = 20
...
class Game {
  ...
  loadLevel(level) {
    ...
    const floorY = this.canvas.height - 20; // ground height
    this.player = new Player(100, floorY - 100); // 100px above ground
  ...
  createLevel(level) {
    const baseY = this.canvas.height - 20; // Ground platform at bottom

    // Always add solid ground
  ...
  update() {
    // Update player with proper canvas height reference
  ...
  drawCircuitElements() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (i % 3 === 0) {
        ...
        for (let j = 0; j < 3; j++) this.ctx.rect(-5 + j * 10, -5, 10, 10);
        this.ctx.moveTo(25, 0); this.ctx.lineTo(15, 0); this.ctx.stroke();
      }
      this.ctx.restore();
    }
  }
  ...
  playSound(type) {
    ...
    try {
      ...
      // ... (keep all existing sound definitions as in original)

      // For brevity, sounds are omitted but should remain as in original
      // Full implementation preserved in your original file
  ...
  setupEventListeners() {
    // ... (all event listeners unchanged from original)
    // Kept identical for brevity — no changes needed here
  }

  setupJoystick() {
    // ... (joystick code unchanged — already functional)
  }
